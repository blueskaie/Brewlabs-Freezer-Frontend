  ...
  <v-dialog v-model="dialog" persistent max-width="450" content-class="br-20">
    <v-card class="br-20">
      ...
      <v-card-text>
        ...
        <div class="caption">
          The specified address will be able to withdraw the univ2 tokens once the unlock date is reached.
        </div>
...
export default {
  ...
  methods: {
    ...
    transferLockOwnership () {
      ...
      TxWrapper.doTransaction(LockerContract.transferLockOwnership(this.unipair.address, this.lock.user_lock_index, this.lock.lockID, this.unlockerAccount), this.$root.$dialog.confirmTx)
        .then(() => {
          this.resolve()
          this.dialog = false
        })
